...
object JsonCodecProvider {
  ...
  /*
  private val cn.playscala.mongo.json.codecs: Map[Class[_], Codec[_]] = Map (
    JsNull.getClass.asInstanceOf[Class[JsNull.type ]] -> new JsNullCodec,
    classOf[JsArray] -> new JsArrayCodec,
  ...
  */
  ...
  //def getCodec(bsonType: BsonType): Codec[JsValue] = cn.playscala.mongo.json.codecs(BSON_TYPE_CLASS_MAP(bsonType)).asInstanceOf[Codec[JsValue]]
  def getCodec[T](bsonType: BsonType): Codec[T] = {
    BSON_TYPE_CODEC_MAP(bsonType).asInstanceOf[Codec[T]]
  }

  //def getCodec(clazz: Class[_]): Codec[JsValue] = cn.playscala.mongo.json.codecs(clazz).asInstanceOf[Codec[JsValue]]
...
class JsonCodecProvider extends CodecProvider {
  override def get[T](clazz: Class[T], registry: CodecRegistry): Codec[T] = {
    //JsonCodecProvider.cn.playscala.mongo.json.codecs(clazz).asInstanceOf[Codec[T]]
